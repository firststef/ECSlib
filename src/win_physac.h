...
static void InitializePhysicsManifolds(PhysicsManifold manifold)
{
    ...
    for (int i = 0; i < manifold->contactsCount; i++)
    {
        ...
        if (MathLenSqr(radiusV) < (MathLenSqr(VECTOR2( gravityForce.x*(float)deltaTime/1000, gravityForce.y*(float)deltaTime/1000 )) + PHYSAC_EPSILON)) manifold->restitution = 0;
    }
}
